...
按当前老猫战法执行某日选股时，该日期被视为 T+1 收盘后的观察日，服务端会回看上一交易日涨停股。例如执行 `2026-05-13` 的选股，会筛选 `2026-05-12` 的首板早盘涨停，并判断 `2026-05-13` 收盘价相对涨停日收盘价的涨幅是否不超过 5%。T+2 开盘是否高开、是否实际买入，由用户手动判断并录入交易系统。
...
- `old_cat_buy`：老猫买入。T+1 收盘后回看上一交易日首板早上封板、非 ST、非一字板候选，按 T+1 涨幅筛出待观察标的。
...
- `old_cat_selection_aligned_8pct`：老猫对照：选股口径对齐 8%。T 日首板早上封板、非 ST、非一字板；按 T+1 收盘相对 T 日收盘涨幅不超过 8% 过滤；T+2 开盘价买入。
- `old_cat_stop_loss_rank`：选股条件与正式老猫战法一致；超过 3 只时按“买入价到分时均线止损价的亏损比例”从低到高排序，止损只亏 0.5% 的优先级高于止损要亏 3% 的。
- `jia_ban`：夹板战法。六个月箱体内有顶有底，允许 2% 突破；T 日收盘接近底部线且跌幅不超过 7%，T+1 开盘买入；跌破底部线止损，止损价高于当日最高价时按收盘价止损；止盈率和持有天数按回测参数执行，单日最多买入止损率最低的 3 只。
