    ...
    def simulate_trade(self, pick: StockPickOut, holding_days: int, take_profit_percent: float) -> BacktestTradeOut | None:
        ...
        for index in range(1, trade_holding_days):
            ...
            if not is_final_day and pick.future_lows[index] < pick.stop_loss_price:
                sell_price = close
                sell_index = index
                exit_reason = "夹板战法：跌破箱体下轨止损"
                break

            if pick.target_price > 0 and high >= pick.target_price:
                sell_price = pick.target_price
                sell_index = index
                exit_reason = "夹板战法：反弹到箱体上轨止盈"
                break
...
@dataclass(frozen=True)
class OldCatLimit2Strategy:
    id: str = "old_cat_limit2"
    ...
    def simulate_trade(self, pick: StockPickOut, holding_days: int, take_profit_percent: float) -> BacktestTradeOut | None:
        ...
        sell_price = pick.future_closes[trade_holding_days - 1]
        sell_index = trade_holding_days - 1
        exit_reason = f"老猫涨停2对比：持有{trade_holding_days}个交易日"

        for index in range(trade_holding_days):
            close = pick.future_closes[index]
            if pick.future_lows[index] < pick.stop_loss_price:
                sell_price = close
                sell_index = index
                exit_reason = "老猫涨停2对比：跌破第一板开盘价止损"
                break

            if pick.future_highs[index] >= take_profit_price:
                sell_price = take_profit_price
                sell_index = index
                exit_reason = f"老猫涨停2对比：涨幅达到{take_profit_percent:.0f}%止盈"
                break
